...
- `smoke_primus_docker.sh` first verifies that the Docker CLI can reach a daemon and that Docker can execute a trivial container, then builds the Primus-enabled GPU image, creates a minimal production-style Primus checkpoint inside the container, loads it through `model_primus`, and runs a small forward pass. Use `DOCKER_GPU_ARGS=""` to run the smoke without `--gpus all` on CPU-only Docker hosts. The preflight defaults to `DOCKER_PREFLIGHT_ARGS="--network=none --security-opt apparmor=unconfined"` and prints the exact Docker failure when daemon access or container execution is unavailable, plus diagnostics for common rootless/user-namespace failures such as missing `newuidmap`/`newgidmap`, `devpts` mount failures, layer `Lchown` failures, and AppArmor policy blocks. See `PR899_DOCKER_HOST_FIX.md` for system-Docker and rootless/user-namespace setup commands. Set `DOCKER_SMOKE_PREFLIGHT=0` only when deliberately debugging the image build on a host where container execution is already understood to be unavailable.
